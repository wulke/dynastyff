...
2. Insert one immutable `picks` row capturing that slot and the selected `player_id`
3. Insert one `roster_players` row assigning current ownership to the drafting team
4. Delete any `user_queue` row for that `(draft_id, player_id)` so queued players disappear once drafted by anyone
5. Commit only if all writes succeed; otherwise roll back the entire pick
