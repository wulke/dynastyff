...
// @spec DFF-BOT-029
test('loadArchetypeConfigFile rejects candidatePoolThreshold values outside the unit interval', () => {
  assert.throws(
    () =>
      loadArchetypeConfigFile({
        configPath: '/virtual/config/archetypes.json',
    ...
    /qb_early\.candidatePoolThreshold must be between 0\.0 and 1\.0\./,
  );
});
