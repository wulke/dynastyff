...
- **Bot Simulator** (`llds/bot-simulator.md`) — drives automated picks for all non-user teams. Before each pick, it may initiate a value-thresholded trade proposal using the current bot's archetype-configured aggressiveness, asset preferences, and stickiness rules; if no viable offer is available, it falls back to pick selection. It uses dynasty value + positional need multiplier + configurable noise across six team archetypes (`bpa`, `balanced`, `win_now`, `punt`, `rb_heavy`, `qb_early`). Emits picks on a 3–5s simulated delay via the Draft Engine. The core pick-selection logic lives in an isomorphic module (`src/draft/bot.ts`) shared with the static build.
