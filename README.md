...
Trade evaluation now also enforces archetype stickiness on outgoing player assets. `rb_heavy` bots protect their top-two RBs, `qb_early` bots protect their highest-value QB, and `win_now` bots protect proven starters (`age >= 27` and `dynasty_value >= 4000`) both when assembling trade fodder and when evaluating incoming offers.
